...
interface VisitState {
  ...
  createVisit: (data: Partial<Visit> & { 
    ...
    target_name?: string;
    target_user_id?: string;  // ✅ NOUVEAU - pour les comptes personnels
  }) => Promise<Visit>;
...
export const useVisitStore = create<VisitState>((set, get) => ({
  ...
  // ============================================================
  // CREATE VISIT - AVEC GESTION DES COMPTES PERSONNELS
  // ============================================================
  createVisit: async (data: Partial<Visit> & { 
    target_type?: 'personal' | 'patient'; 
    target_name?: string;
    target_user_id?: string;  // ✅ NOUVEAU
  }) => {
    try {
      set({ isLoading: true, error: null });
      ...
      // ✅ Déterminer target_user_id (pour les comptes personnels)
      ...
      if (isPonctual) {
        ...
        // ✅ Vérifier le quota sur le COMPTE cible
      ...
      const visitData = {
        user_id: targetUserId || user.id,  // ✅ Utiliser targetUserId si fourni
