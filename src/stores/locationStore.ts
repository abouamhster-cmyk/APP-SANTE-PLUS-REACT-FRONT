...
export const useLocationStore = create<LocationState>((set, get) => ({
  ...
  fetchActiveVisits: async () => {
    try {
      ...
      // 2️⃣ Extraction des IDs uniques pour enrichir les relations
      ...
      if (aidantIds.length > 0) {
        const { data: aidantsData } = await supabase.from('aidants').select('id, user_id, user:profiles!user_id(*)').in('id', aidantIds);
      ...
      // 4️⃣ Construction des locations pour Google Maps (AdvancedMarkers)
      const patients = [
        ...visitsWithRelations.filter(v => v.patient).map(v => v.patient),
        ...ordersWithRelations.filter(o => o.patient).map(o => o.patient)
      ].map(p => ({
        ...p,
        latitude: p.latitude || DEFAULT_LAT,
        longitude: p.longitude || DEFAULT_LNG,
      }));
      ...
      const aidants = [
        ...
        latitude: u.last_latitude || DEFAULT_LAT,
        longitude: u.last_longitude || DEFAULT_LNG,
      })).filter((val, index, self) => self.findIndex(t => t.id === val.id) === index);
