...
const AssignAidantPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // ✅ Récupérer les aidants approuvés avec leur profil
      ...
      if (!linksError && links) {
        // Pour chaque patient, trouver l'aidant assigné (family_id)
        const assignments: Record<string, string> = {};
        for (const link of links) {
          // Vérifier si ce family_id est un aidant
          const isAidant = aidantsData?.some(a => a.user_id === link.family_id);
          if (isAidant) {
