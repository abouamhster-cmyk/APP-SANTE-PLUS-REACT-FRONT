...
const RegisterPage = () => {
  ...
  const renderOfferPreview = () => {
    ...
    if (!offersInitialized || filteredOffers.length === 0) {
      return (
        <div className="rounded-2xl p-4 border" style={{ background: `${primaryColor}08`, borderColor: `${primaryColor}20` }}>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-2xl flex items-center justify-center shrink-0" style={{ background: `${primaryColor}15`, color: primaryColor }}>
              <CreditCard size={16} />
            </div>
            <div>
              <p className="font-bold text-xs" style={{ color: textColor }}>Chargement des offres...</p>
            </div>
          </div>
        </div>
      );
    ...
    return (
      <div className="rounded-2xl p-4 border" style={{ background: `${primaryColor}06`, borderColor: `${primaryColor}20` }}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-xs" style={{ color: textColor }}>Aperçu des abonnements {serviceLabel}</p>
              <p className="text-[10px] text-gray-500">Choix définitif après validation de votre compte.</p>
            </div>
          </div>
          <button type="button" onClick={() => setShowOfferDetails(!showOfferDetails)} className="text-xs font-semibold self-start sm:self-center hover:underline" style={{ color: primaryColor }}>
            {showOfferDetails ? 'Masquer le détail' : 'Voir les détails'}
          </button>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {filteredOffers.slice(0, 4).map((offer: Offer, index: number) => (
            <div key={offer.id || index} className="bg-white rounded-2xl p-2.5 border transition-all" style={{ borderColor: colors.border }}>
              <p className="text-sm">{offer.badge?.split(' ')[0] || '📌'}</p>
              <p className="text-[10px] font-semibold mt-1 truncate" style={{ color: textColor }}>{offer.name}</p>
              <p className="text-[10px] font-bold mt-0.5" style={{ color: primaryColor }}>{Number(offer.price || 0).toLocaleString()} FCFA</p>
              <p className="text-[9px] text-gray-400 mt-0.5">{offer.period || offer.type || 'mois'}</p>
            </div>
          ))}
        </div>
        {showOfferDetails && (
          <div className="mt-3 bg-white rounded-2xl border p-3 space-y-2" style={{ borderColor: colors.border }}>
            {filteredOffers.map((offer: Offer, index: number) => (
              <div key={offer.id || index} className="flex justify-between items-center gap-3 text-[11px] border-b last:border-b-0 pb-1.5 last:pb-0" style={{ borderColor: colors.border }}>
                ...
                <span className="font-bold" style={{ color: primaryColor }}>{Number(offer.price || 0).toLocaleString()} FCFA</span>
              </div>
            ))}
          </div>
        )}
            ...
            <>
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold break-words mt-0.5" style={{ color: textColor }}>Candidature aidant</p>
                </div>
              </div>
              <div className="flex items-start gap-2.5">
                <div className="w-8 h-8 rounded-2xl flex items-center justify-center shrink-0" style={{ background: `${primaryColor}12`, color: primaryColor }}>
            ...
            <>
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold break-words mt-0.5" style={{ color: textColor }}>{isPersonal ? 'Compte personnel sans patient' : 'Accompagnement d\'un proche'}</p>
                </div>
              </div>
              {!isPersonal && (
                <div className="flex items-start gap-2.5">
              ...
              <div className="flex-1 flex flex-col justify-between min-h-0">
                ...
                >
                  ...
                  {step === 1 && (
                    <div className="space-y-3 animate-fadeIn">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs" style={{ color: colors.textLight }}>Sélectionnez le profil qui vous correspond.</p>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <button type="button" onClick={() => handleAccountChoice('family_with_patient')} className="text-left p-4 rounded-2xl border transition-all hover:border-gray-300 flex flex-col justify-between min-h-[135px] sm:min-h-[145px]" style={{ borderColor: accountChoice === 'family_with_patient' ? primaryColor : '#e5e7eb', background: accountChoice === 'family_with_patient' ? `${primaryColor}08` : '#ffffff' }}>
                          ...
                          <div><p className="font-bold text-xs" style={{ color: accountChoice === 'family_with_patient' ? primaryColor : '#111827' }}>Pour un proche</p><p className="text-[11px] text-gray-500 mt-0.5 leading-tight line-clamp-2">Seniors, mamans ou bébés ayant besoin d'assistance.</p></div>
                        </button>
                        <button type="button" onClick={() => handleAccountChoice('personal')} className="text-left p-4 rounded-2xl border transition-all hover:border-gray-300 flex flex-col justify-between min-h-[135px] sm:min-h-[145px]" style={{ borderColor: accountChoice === 'personal' ? primaryColor : '#e5e7eb', background: accountChoice === 'personal' ? `${primaryColor}08` : '#ffffff' }}>
                          ...
                          <div><p className="font-bold text-xs" style={{ color: accountChoice === 'personal' ? primaryColor : '#111827' }}>Compte personnel</p><p className="text-[11px] text-gray-500 mt-0.5 leading-tight line-clamp-2">Créer mon espace sans accompagnement immédiat.</p></div>
                        </button>
                        <button type="button" onClick={() => handleAccountChoice('aidant')} className="text-left p-4 rounded-2xl border transition-all hover:border-gray-300 flex flex-col justify-between min-h-[135px] sm:min-h-[145px]" style={{ borderColor: accountChoice === 'aidant' ? primaryColor : '#e5e7eb', background: accountChoice === 'aidant' ? `${primaryColor}08` : '#ffffff' }}>
                          ...
                          <div><p className="font-bold text-xs" style={{ color: accountChoice === 'aidant' ? primaryColor : '#111827' }}>Devenir aidant</p><p className="text-[11px] text-gray-500 mt-0.5 leading-tight line-clamp-2">Proposer mes services et rejoindre le réseau.</p></div>
                        </button>
                      </div>
                    </div>
                  )}
                  ...
                  {step === 2 && isFamilyWithPatient && (
                    <div className="space-y-3 animate-fadeIn">
                      <div className="space-y-0.5">
                        <h3 className="text-base font-bold" style={{ color: textColor }}>Qui accompagnerons-nous ?</h3>
                        <p className="text-xs" style={{ color: colors.textLight }}>Choisissez la catégorie concernée.</p>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                        <button type="button" onClick={() => handleServiceChoice('senior')} className="p-4 rounded-2xl border text-left transition-all hover:border-gray-300 flex flex-col justify-between min-h-[130px] sm:min-h-[140px]" style={{ borderColor: formData.patientCategory === 'senior' ? primaryColor : '#e5e7eb', background: formData.patientCategory === 'senior' ? `${primaryColor}08` : '#ffffff' }}>
                          ...
                          <div><p className="font-bold text-xs" style={{ color: textColor }}>Senior</p><p className="text-[11px] text-gray-500 mt-0.5 leading-tight line-clamp-2">Accompagnement quotidien d'une personne âgée à domicile.</p></div>
                        </button>
                        <button type="button" onClick={() => handleServiceChoice('maman_bebe')} className="p-4 rounded-2xl border text-left transition-all hover:border-gray-300 flex flex-col justify-between min-h-[130px] sm:min-h-[140px]" style={{ borderColor: formData.patientCategory === 'maman_bebe' ? primaryColor : '#e5e7eb', background: formData.patientCategory === 'maman_bebe' ? `${primaryColor}08` : '#ffffff' }}>
                          ...
                          <div><p className="font-bold text-xs" style={{ color: textColor }}>Maman & Bébé</p><p className="text-[11px] text-gray-500 mt-0.5 leading-tight line-clamp-2">Aide post-partum, soins bébé et soutien de la maman.</p></div>
                        </button>
                      </div>
                    </div>
                  )}
                  ...
                  {((step === 2 && !isFamilyWithPatient) || (step === 3 && isFamilyWithPatient)) && (
                    <div className="space-y-3 animate-fadeIn">
                      <div className="space-y-0.5"><h3 className="text-base font-bold" style={{ color: textColor }}>Création de vos identifiants</h3><p className="text-xs" style={{ color: colors.textLight }}>Vos informations de connexion personnelles.</p></div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                        ...
                        <div className="sm:col-span-2"><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Adresse e-mail *</label><div className="relative"><Mail className="absolute left-3 top-1/2 -translate-y-1/2 size-4" style={{ color: colors.textLight }} /><input name="email" type="email" value={formData.email} onChange={handleChange} placeholder="votremail@adresse.com" className="w-full pl-9 pr-3.5 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }} required /></div></div>
                        <div className="sm:col-span-2"><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Mot de passe *</label><div className="relative"><Lock className="absolute left-3 top-1/2 -translate-y-1/2 size-4" style={{ color: colors.textLight }} /><input type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleChange} required minLength={6} placeholder="Minimum 6 caractères" className="w-full pl-9 pr-9 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }} /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color: colors.textLight }}>{showPassword ? <EyeOff size={16} /> : <Eye size={16} />}</button></div></div>
                      </div>
                    </div>
                  )}
                  ...
                  {step === 3 && isPersonal && (
                    <div className="space-y-3 animate-fadeIn">
                      <div className="space-y-0.5"><h3 className="text-base font-bold" style={{ color: textColor }}>Dernière vérification</h3><p className="text-xs" style={{ color: colors.textLight }}>Relisez vos informations avant de valider.</p></div>
                  ...
                  {step === 3 && isAidant && (
                    <div className="space-y-3 animate-fadeIn">
                      <div className="space-y-0.5"><h3 className="text-base font-bold" style={{ color: textColor }}>Votre expérience</h3><p className="text-xs" style={{ color: colors.textLight }}>Renseignez vos spécialités d'accompagnement.</p></div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                        <div><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Date de naissance</label><input type="date" name="birth_date" value={aidantData.birth_date} onChange={(e) => setAidantData({ ...aidantData, birth_date: e.target.value })} className="w-full px-3.5 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }} /></div>
                        <div><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Années d'expérience</label><select name="experience_years" value={aidantData.experience_years} onChange={(e) => setAidantData({ ...aidantData, experience_years: e.target.value })} className="w-full px-3.5 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }}><option value="">Sélectionner</option><option value="0">Débutant / &lt; 1 an</option><option value="2">2 à 3 ans</option><option value="4">4 à 5 ans</option><option value="6">6 à 10 ans</option><option value="10">Plus de 10 ans</option></select></div>
                        <div className="sm:col-span-2"><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Adresse de résidence *</label><div className="relative"><MapPin className="absolute left-3 top-1/2 -translate-y-1/2 size-4" style={{ color: colors.textLight }} /><input name="address" value={aidantData.address} onChange={(e) => setAidantData({ ...aidantData, address: e.target.value })} placeholder="Quartier, Ville" className="w-full pl-9 pr-3.5 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }} required /></div></div>
                      </div>
                      <div><p className="text-xs font-semibold mb-1.5" style={{ color: textColor }}>Spécialités principales</p><div className="grid grid-cols-2 gap-2">{SPECIALTIES.map((item) => { const active = aidantData.specialties.includes(item.id); return <button type="button" key={item.id} onClick={() => toggleSpecialty(item.id)} className="px-3 py-2.5 rounded-2xl border text-left text-xs transition-all hover:border-gray-300" style={{ borderColor: active ? primaryColor : '#e5e7eb', background: active ? `${primaryColor}08` : '#ffffff', color: textColor }}><span className="mr-2">{item.icon}</span>{item.label}</button>; })}</div></div>
                    </div>
                  )}
                  ...
                  {step === 4 && isFamilyWithPatient && (
                    <div className="space-y-3 animate-fadeIn">
                      <div className="space-y-0.5"><h3 className="text-base font-bold" style={{ color: textColor }}>Le proche à accompagner</h3><p className="text-xs" style={{ color: colors.textLight }}>Service ciblé : {serviceLabel}</p></div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Prénom *</label><input name="patient.first_name" value={formData.patientData.first_name} onChange={handleChange} className="w-full px-3.5 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }} required /></div>
                        <div><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Nom de famille *</label><input name="patient.last_name" value={formData.patientData.last_name} onChange={handleChange} className="w-full px-3.5 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }} required /></div>
                        <div><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Âge (ans)</label><input name="patient.age" type="number" value={formData.patientData.age} onChange={handleChange} className="w-full px-3.5 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }} /></div>
                        <div><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Sexe</label><select name="patient.gender" value={formData.patientData.gender} onChange={handleChange} className="w-full px-3.5 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }}><option value="">Sélectionner</option><option value="male">Homme</option><option value="female">Femme</option></select></div>
                        <div className="sm:col-span-2"><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Lieu de prise en charge *</label><div className="relative"><MapPin className="absolute left-3 top-1/2 -translate-y-1/2 size-4" style={{ color: colors.textLight }} /><input name="patient.address" value={formData.patientData.address} onChange={handleChange} placeholder="Adresse précise ou repères utiles" className="w-full pl-9 pr-3.5 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }} required /></div></div>
                        <div><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Téléphone (optionnel)</label><div className="relative"><Phone className="absolute left-3 top-1/2 -translate-y-1/2 size-4" style={{ color: colors.textLight }} /><input name="patient.phone" value={formData.patientData.phone} onChange={handleChange} className="w-full pl-9 pr-3.5 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }} /></div></div>
                        <div><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Contact d'urgence</label><div className="relative"><Phone className="absolute left-3 top-1/2 -translate-y-1/2 size-4" style={{ color: colors.textLight }} /><input name="patient.emergency_contact" value={formData.patientData.emergency_contact} onChange={handleChange} className="w-full pl-9 pr-3.5 py-2.5 rounded-2xl border outline-none text-xs focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }} /></div></div>
                        <div className="sm:col-span-2"><label className="block text-xs font-semibold mb-1" style={{ color: textColor }}>Consignes ou besoins particuliers</label><textarea name="patient.notes" value={formData.patientData.notes} onChange={handleChange} rows={2} placeholder="Traitements, régimes, allergies, autonomie..." className="w-full px-3.5 py-2 rounded-2xl border outline-none text-xs resize-none focus:ring-1" style={{ borderColor: colors.border, background: colors.background, color: textColor }} /></div>
                      </div>
                    </div>
                  )}
                  ...
                  {step === 4 && isAidant && (
                    <div className="space-y-3 animate-fadeIn">
                      <div className="space-y-0.5"><h3 className="text-base font-bold" style={{ color: textColor }}>Mobilité et présentation</h3><p className="text-xs" style={{ color: colors.textLight }}>Où et quand pouvez-vous intervenir ?</p></div>
                      <div><p className="text-xs font-semibold mb-1.5" style={{ color: textColor }}>Zones d'intervention</p><div className="flex flex-wrap gap-2">{ZONES.map((zone) => { const active = aidantData.zones.includes(zone); return <button type="button" key={zone} onClick={() => toggleZone(zone)} className="px-3.5 py-1.5 rounded-full text-xs font-medium border transition-all" style={{ borderColor: active ? primaryColor : '#e5e7eb', background: active ? primaryColor : '#ffffff', color: active ? '#ffffff' : '#4b5563' }}>{zone}</button>; })}</div></div>
                ...
                <div className="space-y-2 mt-4 pt-4 border-t" style={{ borderColor: colors.border }}>
                  ...
                  <label className="flex items-start gap-2.5 cursor-pointer">
                    ...
                    <span className="text-xs leading-relaxed" style={{ color: colors.textLight }}>
                      J'accepte sans réserve les <button type="button" onClick={() => setShowCGU(true)} className="font-bold hover:underline" style={{ color: primaryColor }}>Conditions Générales d'Utilisation</button> de Santé Plus.
                    </span>
                  </label>
